...
def perfect_square(num):
    ...
    assert num >=0, "num shouldn't be negtive"
    root = 1
    while True:
        square = root * root
        if square == num:
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy(my_score, oppo_score):
        return n
    return strategy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    my_score = 0
    oppo_score = 0
    same_score = strategy(0, 0)
    while my_score <= 100:
        oppo_score = 0
        while oppo_score <= 99:
            if same_score != strategy(my_score, oppo_score):
                return False
            oppo_score += 1
        my_score += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    min_roll_num = 11
    max_score = -1
    while test_roll_num <= 10:
        score = avg_roll_dice(test_roll_num, dice)
        if score > max_score:
            max_score = score
            min_roll_num = test_roll_num
        test_roll_num += 1
    return min_roll_num
    # END PROBLEM 9
